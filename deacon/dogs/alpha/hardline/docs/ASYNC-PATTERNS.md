# Async Patterns for Hardline Contributors

## Overview

Hardline migrated from synchronous `rusqlite` to asynchronous `sqlx` in version 0.3.0. All database operations and command handlers now use async/await patterns with Tokio runtime.
...
```rust
// crates/hardline/src/commands/status/mod.rs
