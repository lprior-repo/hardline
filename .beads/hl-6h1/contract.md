...
### Feature
Fix a Time-Of-Check-Time-Of-Use (TOCTOU) race condition in the cross-process lock acquisition path for workspace creation. The `.hardline` data directory is created BEFORE the exclusive file lock is acquired, leaking a "half-initialized" signal to concurrent processes and recovery logic.
...
| **Lock directory** | The directory containing the lock file (currently `.hardline/`) |
| **Data directory** | The `.hardline/` directory that holds runtime state, databases, and config |
...
acquire_cross_process_lock()
  |
  +-> create_dir_all(.hardline)    |
  |   [RACE WINDOW STARTS]        |
  |   *** CRASH HERE ***          |
  |                                acquire_cross_process_lock()
  |                                  |
  |                                +-> create_dir_all(.hardline)  [no-op, already exists]
  |                                +-> open(.hardline/workspace-create.lock)
  |                                +-> try_lock_exclusive()     [SUCCEEDS - A never locked it]
  |                                +-> [proceeds with workspace creation]
  |                                +-> [BUT .hardline was a phantom from A's crash]
```

### Actual Impact

The race window exists between:
1. `tokio::fs::create_dir_all(&lock_dir)` (line 133) - creates `.hardline`
2. `acquire_file_lock_with_timeout(&file, ...)` (line 148) - acquires file lock

If Process A crashes between these two operations:
- `.hardline` directory persists as a phantom signal
- Other code checking `.hardline` existence (`recovery.rs:57`, `workspace_integrity/checks.rs:236`) gets a false positive
...
- The `.hardline` directory name is a convention, not a hard requirement for lock placement
- `OpenOptions::new().create(true)` creates the file but NOT parent directories

### Open Questions
1. Should the lock file path change from `.hardline/workspace-create.lock` to a repo-root-level path (e.g., `.scp-workspace-create.lock`) to fully decouple lock from data? **Recommendation: Yes.**
...
3. Is there any code that relies on `.hardline` directory creation as a signal for "init in progress"? **Answer: `recovery.rs:57` checks `.hardline` existence to decide whether to log.**
...
- **P2.1**: The exclusive file lock MUST be held BEFORE any data directory (`.hardline`) creation
- **P2.2**: The data directory MUST NOT exist OR the lock holder is the one who created it

### P3: Cleanup Preconditions
- **P3.1**: If lock acquisition fails, the `.hardline` directory MUST NOT have been created
...
- **Q1.3**: On success, returns `Ok(File)` with exclusive lock held. Does NOT create `.hardline` directory — that is the caller's responsibility via `ensure_data_directory()`
...
- **Q2.1**: `.hardline` directory exists if and only if the lock is held by the creating process
- **Q2.2**: `.hardline` directory is never in a "half-initialized" state visible to other processes

### Q3: Failure Postconditions
- **Q3.1**: On lock acquisition failure (timeout), the `.hardline` directory MUST NOT have been created by this call
- **Q3.2**: On directory creation failure after lock acquisition, the lock is released (File dropped) and error is returned
- **Q3.3**: On any error, no phantom `.hardline` directory is left behind
...
FOR ALL calls to acquire_cross_process_lock:
  holding(lock) IMPLIES .hardline directory exists
  .hardline directory exists IMPLIES holding(lock) OR lock was recently released
```

### I2: No Phantom Directory Invariant
```
FOR ALL failure paths:
  returned Err(_) IMPLIES .hardline directory state is unchanged from before the call
```

### I3: Atomic Visibility Invariant
```
FOR ALL concurrent processes:
  Process B observes .hardline directory
  IMPLIES
  the lock that created .hardline was either held or recently released
...
| Condition | Behavior |
|---|---|
| Filesystem does not support advisory locks | Log warning, set `lock_supported = false`, continue unless `Hardline_STRICT_LOCKS` env var set |
| `Hardline_STRICT_LOCKS` set + unsupported FS | Return `Error::State(StateErrorKind::ValidationError)` |

### E3: New Error Variant Needed (Post-Fix)

| Error Variant | When | Purpose |
|---|---|---|
| `Error::Io(IoErrorKind::IoError("Failed to create data directory: {e}"))` | `.hardline` dir creation fails AFTER lock acquired | Distinguish from lock failures; signals lock-was-held-but-dir-failed |
...
```rust
/// ACQUIRES lock AND creates .hardline directory as side effect
/// TOCTOU: .hardline created BEFORE lock held
...
/// (NOT inside .hardline/ to avoid chicken-and-egg TOCTOU)
///
/// Post-condition: Returns Ok(File) with exclusive lock held.
/// Does NOT create .hardline directory. Caller must call
/// ensure_data_directory() AFTER acquiring the lock.
pub async fn acquire_cross_process_lock(repo_root: &Path) -> Result<File>

/// Create .hardline data directory.
///
...
///
/// POSTCONDITION: .hardline directory exists.
...
///   2. acquire_cross_process_lock()         // creates .hardline + locks  [TOCTOU HERE]
///   3. create_dir_all(parent)               // creates workspace dir
///   4. jj workspace add                     // creates workspace
///
/// Fixed:
///   1. acquire_lock_with_backoff()          // in-process lock
///   2. acquire_cross_process_lock()         // lock file at repo root, NO .hardline creation
///   3. ensure_data_directory()              // creates .hardline WHILE LOCKED
...
**Current**: Lock file at `{repo_root}/.hardline/workspace-create.lock`
**Fixed**: Lock file at `{repo_root}/.scp-workspace-create.lock`

**Rationale**: The lock file must reside in a directory that is guaranteed to exist (`repo_root`). Placing it inside `.hardline` creates a circular dependency: you need `.hardline` to open the lock file, but you need the lock to safely create `.hardline`.

### Change 2: Remove create_dir_all from acquire_cross_process_lock (jj_lock.rs)

**Current** (lines 132-135):
```rust
let lock_dir = repo_root.join(".hardline");
...
New function that creates `.hardline` directory, to be called AFTER lock is held:
```rust
pub async fn ensure_data_directory(repo_root: &Path) -> Result<()> {
    let data_dir = repo_root.join(".hardline");
...
### Change 6: Update all tests referencing .hardline path for lock file
...
- **NG4**: Do NOT change the `Hardline_STRICT_LOCKS` environment variable handling
...
```
Process A: create_dir_all(.hardline) -> CRASH
Process B: sees .hardline exists -> assumes init in progress -> waits/block/detects stale state
FIX: .hardline never created unless lock is held
```

### V2: Concurrent Directory Creation
```
Process A: create_dir_all(.hardline) -> (slow FS)
Process B: create_dir_all(.hardline) -> (slow FS)
Both: proceed to lock acquisition, one wins, one times out
BUT: .hardline was created by both, potential metadata corruption
FIX: Only one process holds lock when creating .hardline
```

### V3: Recovery False Positive
```
Process A: create_dir_all(.hardline) -> CRASH before lock
recovery.rs:57: sees .hardline exists -> tries to log recovery -> creates recovery.log
Doctor command: sees .hardline + recovery.log -> thinks system was initialized -> skips init
User: runs init -> "already initialized" -> confused
FIX: .hardline only exists when lock was successfully acquired
```
