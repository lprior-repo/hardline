...
- Lock file now lives at repo root (not inside `.isolate/`), eliminating the chicken-and-egg TOCTOU

**Change 2: Remove `create_dir_all` from `acquire_cross_process_lock` (lines 131-139)**
- Removed `create_dir_all(&lock_dir)` that created `.isolate/` before lock acquisition
...
- Creates `.isolate/` directory via `tokio::fs::create_dir_all`
- Error mapped to `Error::io_error("Failed to create data directory: {e}")`
- Documented precondition (caller MUST hold lock) and postcondition (`.isolate` exists)
...
- Ensures `.isolate/` is only created while cross-process lock is held
...
| Q1.3: `.isolate` created AFTER lock acquired | PASS | `ensure_data_directory` called after `acquire_cross_process_lock` in `create_workspace_synced` |
| Q3.1: On lock failure, `.isolate` NOT created | PASS | `create_dir_all` removed from `acquire_cross_process_lock` |
...
|---|---|---|
| I1: Lock-Before-Create | PASS | `.isolate` creation is only in `ensure_data_directory`, called after lock held |
