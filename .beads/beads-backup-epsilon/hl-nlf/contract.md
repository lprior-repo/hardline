...
## Context
- **Feature**: SVT Load Test 5 (svt_batch_5)
- **Domain terms**: SVT (Super Velocity Throughput), svt-runner.sh, batch execution, opencode serve
- **Bead ID**: hl-nlf
- **Bead Title**: svt_batch_5
- **Phase**: contract-synthesis

### Assumptions
1. SVT batch 5 uses a specific batch configuration (likely batch_size=5 or a scaled load profile)
...
5. This is a load test running multiple beads in parallel to test throughput

### Open Questions
- What specific batch_size does "batch 5" use? (Inferred: likely 5 concurrent beads)
- What is the expected success criteria? (Inferred: all beads complete without errors)
- Is this testing a specific failure scenario or throughput baseline?
...
| P7 | There is at least one ready bead to process | Runtime-checked | `bd ready --json` returns non-empty |
| P8 | Available ports for opencode serve instances | Runtime-checked | Port availability check |
...
- **Error::PortUnavailable** - When no ports are available for opencode serve instances
...
```rust
/// Run SVT load test batch 5
/// Returns a report with execution results for all beads in the batch
fn run_svt_batch5(target_dir: &Path) -> Result<SvtBatchReport, Error>;
...
| P7: ready beads exist | Runtime-checked | `Result<Vec<Bead>, Error::NoReadyBeads>` |
| P8: ports available | Runtime-checked | `Result<PortPool, Error::PortUnavailable>` |
...
- **VIOLATES P7**: Running SVT when no beads are ready -- should produce `Err(Error::NoReadyBeads("no ready beads found"))`
- **VIOLATES P8**: Running SVT when all ports (3000-3010) are blocked -- should produce `Err(Error::PortUnavailable("no ports available"))`
