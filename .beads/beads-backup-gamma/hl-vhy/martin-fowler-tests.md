...
## Overview
This test plan covers SVT Load Test 3 (hl-vhy) - the third iteration of batch load testing for the Super Velocity Throughput (SVT) pipeline. Tests are organized following Martin Fowler's Given-When-Then pattern with expressive names that serve as executable specifications.

## Happy Path Tests

### test_batch_3_script_exists_and_is_executable
...
### test_opencode_server_starts_successfully_on_available_port
...
### test_session_created_for_ready_bead_in_batch
...
### test_agent_dispatch_succeeds_in_batch_context
...
### test_poll_completes_within_timeout_for_batch
...
### test_batch_report_generated_with_execution_matrix
Given: All sessions have completed processing
When: Report generation is triggered
Then: JSON report is produced with port, session_id, status, duration, batch_id fields

### test_servers_cleaned_up_on_batch_exit
...
### test_batch_3_completes_with_zero_exit_code
Given: All preconditions are met and beads are processed
When: svt-runner.sh completes full execution with batch_id=3
Then: Script exits with code 0

### test_batch_id_preserved_in_execution
Given: Batch execution is initiated with batch_id=3
When: Report is generated
Then: Report contains batch_number = 3
...
### test_returns_error_when_no_ready_beads_for_batch
...
### test_returns_error_when_server_start_fails_for_batch
...
### test_returns_error_when_session_creation_fails_in_batch
...
### test_returns_error_with_invalid_batch_configuration
Given: Batch configuration has wrong batch_id (not 3)
When: validate_batch_config is called
Then: Returns `Err(Error::BatchConfigInvalid("batch_id must be 3"))`

### test_returns_error_on_concurrent_session_conflict
Given: Race condition in session management
When: Concurrent session creation conflicts
Then: Returns `Err(Error::ConcurrentSessionConflict)`

## Edge Case Tests

### test_handles_single_bead_batch_iteration_3
Given: Only one ready bead exists in the system
When: SVT runs with batch_size=1 and batch_id=3
Then: Processes exactly one bead, generates report with batch_id=3

### test_handles_empty_bead_list_gracefully_in_batch_3
Given: No beads are available for processing
When: SVT batch 3 runs
Then: Generates empty report with batch_id=3, exits cleanly with code 0

### test_handles_port_increment_for_multiple_servers_in_batch
...
### test_handles_server_crash_during_batch_execution
...
### test_handles_api_timeout_gracefully_in_batch
...
### test_handles_invalid_bead_id_in_batch_3
...
### test_handles_concurrent_session_creation_in_batch
...
### test_handles_report_with_mixed_statuses_in_batch_3
Given: Some beads succeed, some fail during processing
When: Report is generated for batch 3
Then: Report accurately reflects each bead's status with batch_id=3

### test_handles_partial_batch_completion
Given: Some beads in batch complete, others timeout
When: Batch execution ends with partial success
Then: Report shows mixed results, exit code reflects partial failure
...
### test_precondition_p6_ready_beads_exist
Given: Bead system state
When: Checking for ready beads
Then: Precondition P6 enforced via `bd ready` check

### test_precondition_p7_batch_config_valid
Given: Batch configuration
When: Validating batch_id=3
Then: Precondition P7 enforced via config validation
...
### test_postcondition_q6_batch_id_in_report
Given: Report generation
When: Checking batch metadata
Then: Postcondition Q6 verified by batch_number=3 in report
...
### test_invariant_i6_batch_iteration_preserved
Given: Batch execution
When: Throughout execution
Then: Invariant I6 maintained: batch_id=3 in all operations

### test_invariant_i7_concurrent_sessions_isolated
Given: Multiple concurrent sessions
When: Processing beads
Then: Invariant I7 maintained: sessions do not interfere
...
### test_violation_p6_returns_no_ready_beads_error
Given: No beads in ready state
When: bd ready returns empty
Then: Returns `Err(Error::NoReadyBeads)` - NOT a panic

### test_violation_p7_returns_batch_config_invalid_error
Given: Batch configuration with batch_id != 3
When: validate_batch_config is called
Then: Returns `Err(Error::BatchConfigInvalid("batch_id must be 3"))` - NOT a panic
...
### test_violation_q6_returns_report_generation_failed_error
Given: Report generation without batch_id
When: generate_batch_report is called
Then: Returns `Err(Error::ReportGenerationFailed("batch_id not in report"))` - NOT a panic

## Given-When-Then Scenarios

### Scenario 1: Successful SVT Batch 3 Execution with Single Ready Bead
Given:
...
- Batch configuration is valid (batch_id=3)

When:
- svt-runner.sh is executed with batch_id=3 and batch_size=1
...
- JSON report is generated with execution matrix including batch_number=3
- All servers are cleaned up
- Exit code is 0

### Scenario 2: SVT Batch 3 Fails Due to Missing Dependency
Given:
- svt-runner.sh script is missing from expected path

When:
- SVT batch 3 test execution is attempted
...
### Scenario 3: SVT Batch 3 Fails Due to No Ready Beads
Given:
...
When:
- SVT batch 3 test execution attempts to discover beads
...
### Scenario 4: SVT Batch 3 Recovers from Port Conflict
Given:
...
- Batch ID 3 is preserved in report
- Exit code is 0

### Scenario 5: SVT Batch 3 Handles Server Crash and Cleans Up
Given:
...
### Scenario 6: SVT Batch 3 Validates Configuration
Given:
- Batch configuration with batch_id=3

When:
- validate_batch_config is called

Then:
- Configuration is accepted
- Execution proceeds with batch_id=3

### Scenario 7: SVT Batch 3 Rejects Invalid Configuration
Given:
- Batch configuration with batch_id != 3 (e.g., batch_id=1 or batch_id=5)

When:
- validate_batch_config is called

Then:
- Error::BatchConfigInvalid is returned
- Execution does not proceed

## Test Execution Order

1. **Dependency Validation Tests** (prerequisite checks)
2. **Configuration Validation Tests** (batch_id=3 validation)
3. **Happy Path Tests** (full successful execution)
4. **Error Path Tests** (each failure mode in isolation)
5. **Edge Case Tests** (boundary conditions)
6. **Contract Verification Tests** (pre/post/invariant enforcement)
7. **Contract Violation Tests** (error handling for violations)
8. **Given-When-Then Scenarios** (end-to-end workflows)
...
- Batch ID 3 is preserved throughout execution
