    ...
    async fn check_stale_locks(workspace_path: &Path) -> Result<Option<IntegrityIssue>> {
        ...
        if lock_exists {
            ...
            let age = SystemTime::now()
                ...
                .map_err(|e| Error::Internal(format!("Failed to calculate lock age: {e}")))?;
...
impl RepairExecutor {
    ...
    pub async fn repair(&self, validation: &ValidationResult) -> Result<RepairResult> {
        ...
        let strategy = validation
            ...
            .ok_or_else(|| Error::Internal("No issues found in validation result".to_string()))?;
    ...
    async fn forget_and_recreate(
        ...
        let root = workspace_path
            ...
            .ok_or_else(|| Error::Internal("Could not determine repository root".to_string()))?;
