...
use scp_core::infrastructure::database::{SqliteDatabaseService, DatabaseConfig, DatabaseService};
use std::env;
...
{
    let rt = Runtime::new().map_err(|e| scp_core::Error::internal(format!("Failed to create runtime: {}", e)))?;
    rt.block_on(f)
}
...
pub(crate) fn acquire_with_path(session: &str, agent: &str, ttl: Option<u64>, db_path: &str) -> Result<()> {
    run_async(async {
        ...
        let res = mgr.lock_with_ttl(session, agent, ttl.unwrap_or(0)).await?;
        println!("Lock acquired: {} for agent {} (expires: {})", res.session, res.agent_id, res.expires_at);
        Ok(())
    })
}
...
pub(crate) fn heartbeat_with_path(session: &str, agent: &str, db_path: &str) -> Result<()> {
    run_async(async {
        ...
        println!("Heartbeat sent: {} (new expiration: {})", session, res.expires_at);
        Ok(())
    })
}
...
pub(crate) fn status_with_path(session: &str, db_path: &str) -> Result<()> {
    run_async(async {
        ...
        match res.holder {
            Some(agent) => println!("Locked: session {} held by {} (expires: {})", session, agent, res.expires_at.expect("expires_at missing")),
            None => println!("Unlocked: session {}", session),
        }
        Ok(())
    })
}
...
pub(crate) fn list_with_path(db_path: &str) -> Result<()> {
    run_async(async {
        ...
        if locks.is_empty() {
            ...
            println!("{:-<65}", "");
            for lock in locks {
                println!("{:<20} {:<20} {:<25}", lock.session, lock.agent_id, lock.expires_at);
            }
        }
        Ok(())
    })
