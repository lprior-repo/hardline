...
fn check_condition(session_name: &str, mode: &WaitMode) -> Result<bool> {
    ...
    match mode {
        ...
        WaitMode::Healthy => {
            // Healthy means the workspace exists and is not in a failed/completed state
            match workspace {
                Some(ws) => {
                    // Check if workspace is in a good state
                    // For now, any existing workspace is considered healthy
                    Ok(true)
                }
                None => Ok(false),
            }
        }
        WaitMode::Status(expected) => {
            match workspace {
                Some(ws) => {
                    // Compare branch name as proxy for status
                    // In a real implementation, we'd check actual session state
                    Ok(ws.branch.contains(expected))
                }
                None => Ok(false),
            }
        }
