...
mod tests {
    ...
    use crate::commands::lock::{
        acquire_with_path, release_with_path, heartbeat_with_path,
        status_with_path, list_with_path
    };
    ...
    fn heartbeat_from_wrong_agent_fails() {
        ...
        assert!(res.is_err(), "Expected error from wrong agent, got {:?}", res);
    }
    ...
    fn acquire_with_empty_session_fails() {
        ...
        assert!(res.is_err(), "Expected error on empty session, got {:?}", res);
    }
    ...
    fn acquire_with_too_long_session_fails() {
        ...
        let res = acquire_with_path(&session, "a1", None, path);
        assert!(res.is_err(), "Expected error on too long session, got {:?}", res);
    }
    ...
    fn heartbeat_for_expired_lock_fails() {
        ...
        let res = heartbeat_with_path("s1", "a1", path);
        assert!(res.is_err(), "Expected error on expired lock, got {:?}", res);
    }
