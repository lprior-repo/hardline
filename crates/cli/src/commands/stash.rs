...
pub fn save(message: Option<&str>, include_untracked: bool, _patch: bool) -> Result<()> {
    ...
    let repo = repository::open(&cwd)
        .map_err(|e| Error::VcsConflict("git stash".to_string(), e.to_string()))?;
...
pub fn pop(stash_ref: Option<&str>, _restore_index: bool) -> Result<()> {
    ...
    let repo = repository::open(&cwd)
        .map_err(|e| Error::VcsConflict("git stash pop".to_string(), e.to_string()))?;
...
pub fn list() -> Result<()> {
    ...
    let repo = repository::open(&cwd)
        .map_err(|e| Error::VcsConflict("git stash list".to_string(), e.to_string()))?;

    let entries = stash::list(&repo)
        .map_err(|e| Error::VcsConflict("git stash list".to_string(), e.to_string()))?;
...
pub fn drop(stash_ref: &str, _force: bool) -> Result<()> {
    ...
    let repo = repository::open(&cwd)
        .map_err(|e| Error::VcsConflict("git stash drop".to_string(), e.to_string()))?;
    ...
    let index = stash_ref
        ...
        .ok_or_else(|| {
            Error::VcsConflict(
                ...
                "Invalid stash reference".to_string(),
            )
        })?;
...
pub fn show(stash_ref: Option<&str>, _stat: bool) -> Result<()> {
    ...
    let repo = repository::open(&cwd)
        .map_err(|e| Error::VcsConflict("git stash show".to_string(), e.to_string()))?;
