...
pub fn run(commands: Vec<String>, checkpoint_path: Option<&str>, dry_run: bool) -> Result<()> {
    ...
    // For now, we implement a simple checkpoint using file system
    // In a real implementation, this would use a proper database checkpoint
    ...
    // If failure occurred and we have a checkpoint, try to rollback
    if has_failure {
        if checkpoint_path.is_some() {
            // For now, we just report that rollback would happen
            // In a full implementation, we'd restore from checkpoint
            println!(
            ...
            // No checkpoint, just report failure
...
pub fn run(commands: Vec<String>, checkpoint_path: Option<&str>, dry_run: bool) -> Result<()> {
    ...
    // Create checkpoint if path provided
    let checkpoint: Option<CheckpointGuard> = if let Some(path) = checkpoint_path {
        let path = PathBuf::from(path);
        match AutoCheckpoint::create(&path) {
            Ok(guard) => Some(guard),
            Err(e) => {
    ...
    // If failure occurred and we have a checkpoint, try to rollback
    if has_failure {
        if let Some(mut guard) = checkpoint {
            // Attempt rollback
            if let Err(rollback_err) = guard.restore() {
                return Err(scp_core::Error::BatchRollbackFailed(format!(
                    "failed to restore checkpoint: {}",
                    rollback_err
                )));
            }
            // Report rollback happened
            println!("Batch execution failed. Rolled back to checkpoint.");
            println!("Results:");
            for result in &results {
                let status = if result.success { "✓" } else { "✗" };
                println!("  {} {}: {}", status, result.command, result.output);
            ...
            return Err(scp_core::Error::BatchCommandFailed(
                "batch failed, rolled back".to_string(),
            ));
        } else {
            // No checkpoint, just report failure
            println!("Batch execution failed. No checkpoint to rollback to.");
    ...
    // Success!
    let checkpoint_id = checkpoint.map(|g| g.id().to_string());
    println!("Batch executed successfully ({} commands)", results.len());
    for result in &results {
        let status = if result.success { "✓" } else { "✗" };
        println!("  {} {}", status, result.command);
