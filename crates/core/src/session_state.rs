...
impl SessionStateManager<Active> {
    ...
    pub fn begin_commit_effect(
        mut self,
        reason: impl Into<String>,
    ) -> Result<SessionStateManager<CommittingEffect>> {
        let transition =
            StateTransition::new(SessionState::Active, SessionState::CommittingEffect, reason);
        self.record_transition(&transition)?;
        Ok(SessionStateManager {
            session_id: self.session_id,
            current_state: self.current_state,
...
impl SessionStateManager<CommittingEffect> {
    ...
    pub fn commit_complete(
        mut self,
        reason: impl Into<String>,
    ) -> Result<SessionStateManager<Active>> {
        let transition =
            StateTransition::new(SessionState::CommittingEffect, SessionState::Active, reason);
        self.record_transition(&transition)?;
        Ok(SessionStateManager {
            session_id: self.session_id,
            current_state: self.current_state,
    ...
    pub fn fail(mut self, reason: impl Into<String>) -> Result<SessionStateManager<Failed>> {
        let transition =
            StateTransition::new(SessionState::CommittingEffect, SessionState::Failed, reason);
        self.record_transition(&transition)?;
        Ok(SessionStateManager {
            session_id: self.session_id,
            current_state: self.current_state,
