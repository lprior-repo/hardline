...
impl SqliteWorktreeRow {
    fn to_worktree(&self) -> Worktree {
        ...
        let name = WorktreeName::new(&self.name)
            .unwrap_or_else(|_| WorktreeName::new_unchecked("unknown".to_string()));
        let path = AbsolutePath::new(&self.path).unwrap_or_else(|_| {
        ...
        let parent_path = AbsolutePath::new(&self.parent_path).unwrap_or_else(|_| {
            // Use new_unchecked since /tmp is guaranteed to be valid
            // SAFETY: /tmp is always an absolute path without traversal
            unsafe { AbsolutePath::new_unchecked("/tmp") }
        });
