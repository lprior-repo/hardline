...
#[tokio::test]
    async fn sqlite_repository_integration_worktree_id_uniqueness() {
        ...
        let mut wt1 = create_test_worktree("unique-test-1", "/tmp/wt1", "/home/user/proj", WorktreeTypeEnum::Development, None);
    ...
    async fn setup_initializes_worktrees_table() {
        ...
        let result = sqlx::query("SELECT name FROM sqlite_master WHERE type='table' AND name='worktrees'")
...
#[tokio::test]
    async fn setup_creates_name_unique_constraint() {
        ...
        let result = sqlx::query("SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='worktrees'")
            .fetch_all(repo.pool())
            .await
            .unwrap();
    ...
    async fn setup_creates_state_index() {
        let repo = create_sqlite_repo().await.unwrap();
        let result = sqlx::query("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_worktrees_state'")
            .fetch_optional(repo.pool())
            .await
            .unwrap();
        assert!(result.is_some());
    ...
    async fn setup_creates_type_index() {
        let repo = create_sqlite_repo().await.unwrap();
        let result = sqlx::query("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_worktrees_type'")
            .fetch_optional(repo.pool())
            .await
            .unwrap();
        assert!(result.is_some());
    ...
    async fn save_worktree_updates_existing_entry() {
        ...
        // Update name
        worktree = create_test_worktree("update-test", "/tmp/wt", "/home/user/proj", WorktreeTypeEnum::Testing, Some("main"));
    ...
    async fn find_by_name_with_duplicate_names_uses_on_conflict_update() {
        ...
        let mut worktree2 = create_test_worktree("dup-test", "/tmp/wt2", "/home/user/proj", WorktreeTypeEnum::Testing, None);
...
// This test is now covered by that renamed test
    ...
    async fn error_invalid_path_format() {
        ...
        assert_eq!(
            result.unwrap_err(),
            WorktreeDomainError::InvalidPath("Path is not absolute: invalid-relative-path".to_string())
        );
    }
    ...
    async fn concurrent_read_multiple_worktrees() {
        ...
        for i in 0..10 {
            let repo_clone = repo.clone();
            let id = WorktreeId::from_string(&format!("00000000-0000-0000-0000-00000000000{}", i)).unwrap();
            let handle = tokio::spawn(async move {
                repo_clone.find_by_id(&id).await
            });
            handles.push(handle);
        }
    ...
    async fn list_filtered_by_state_active() {
        ...
        let active_count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM worktrees WHERE state = ?",)
    ...
    async fn list_filtered_by_type() {
        ...
        let dev_count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM worktrees WHERE worktree_type = ?",)
    ...
    async fn offset_limit_simulation() {
        ...
        for _i in 0..5 {
            let wt = create_test_worktree(&format!("offset-test-{}", _i), &format!("/tmp/wt{}", _i), "/home/user/proj", WorktreeTypeEnum::Development, None);
    ...
    async fn integration_concurrent_updates_same_worktree() {
        ...
        // Test metadata persistence with single update first
        {
            ...
            assert_eq!(check.all_metadata().len(), 1, "Should have 1 metadata entry");
        ...
        for i in 0..5 {
            ...
            let check = repo.find_by_id(worktree.id()).await.unwrap().unwrap();
            assert!(check.all_metadata().len() > i, "Should have at least {} metadata entries", i + 1);
        }
        
        let final_wt = repo.find_by_id(worktree.id()).await.unwrap().unwrap();
        // All 5 metadata entries should be present
        assert!(final_wt.all_metadata().len() >= 5, "Expected at least 5 metadata entries, got {}", final_wt.all_metadata().len());
    }
    ...
    async fn integration_database_schema_migration_compatibility() {
        ...
        // Verify table exists
        let table_count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='worktrees'")
            .fetch_one(repo.pool())
            .await
            .unwrap();
        ...
        let column_names: Vec<(String,)> = sqlx::query_as("SELECT name FROM pragma_table_info('worktrees')")
            .fetch_all(repo.pool())
            .await
            .unwrap();
    ...
    async fn integration_index_utilization() {
        ...
        let indexes: Vec<String> = sqlx::query_scalar("SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='worktrees'")
            .fetch_all(repo.pool())
            .await
            .unwrap();
    ...
    async fn integration_worktree_id_uniqueness() {
        ...
        for i in 0..10 {
            let mut wt = create_test_worktree(&format!("unique-id-test-{}", i), &format!("/tmp/wt{}", i), "/home/user/proj", WorktreeTypeEnum::Development, None);
            repo.save(&mut wt).await.unwrap();
        }
